...
def mlp():
		...
		#return Enc, Dec_a, Dec_b, Dec_c, Dec_d, Disc
...
def _multihead_attention(keys,
		...
		with tf.variable_scope("multihead_attention", reuse=reuse):
				# Linear projections
				proj = tf.layers.dense(keys, num_units * num_heads,
															 activation=lrelu)  # (N, c*h)
				attn_weights = tf.layers.dense(
						keys, num_units * num_heads, activation=lrelu)  # (N, c*h)

				# Split and concat
				proj_ = tf.concat(tf.split(proj, num_heads, axis=1),
													axis=0)  # (h*N, c)
				attn_weights_ = tf.concat(
						tf.split(attn_weights, num_heads, axis=1), axis=0)  # (h*N, c)
				...
				outputs = tf.reduce_sum(tf.multiply(
						proj_, attn_weights_), 1, keep_dims=False)  # (h*N)

				# Restore shape
				outputs = tf.concat(tf.split(tf.expand_dims(
						outputs, 1), num_heads, axis=0), axis=1)  # (N, h)
