...
source_train_data, source1_train_data, target_train_data, source_test_data, source1_test_data, target_test_data, \
...
Enc, Dec_a, Dec_b, Dec_c, Disc = utils1.get_models(model_name)
...
def enc_dec(input, AE_type, is_training=True):
    ...
    if AE_type == "VAE":
        return c_mu, c_log_sigma_sq, c, rec_a, rec_b, rec_c
    if AE_type == "standard":
        return c, rec_a, rec_b, rec_c
...
# encode & decode
if AE_type == "VAE":
    c_mu_a, c_log_sigma_sq_a, c_a, rec_a, _, _ = enc_dec(input_a, AE_type)
    c_mu_b, c_log_sigma_sq_b, c_b, _, rec_b, _ = enc_dec(input_b, AE_type)
    c_mu_c, c_log_sigma_sq_c, c_c, _, _, rec_c = enc_dec(input_c, AE_type)

    _, _, c_a1, rec_a1, _, _ = enc_dec(input_a, AE_type, is_training=False)
    _, _, c_b1, _, rec_b1, _ = enc_dec(input_b, AE_type, is_training=False)
    _, _, c_c1, _, _, rec_c1 = enc_dec(input_c, AE_type, is_training=False)


else:
    c_a, rec_a, _, _ = enc_dec(input_a, AE_type)
    c_b, _, rec_b, _ = enc_dec(input_b, AE_type)
    c_c, _, _, rec_c = enc_dec(input_c, AE_type)

    c_a1, rec_a1, _, _ = enc_dec(input_a, AE_type, is_training=False)
    c_b1, _, rec_b1, _ = enc_dec(input_b, AE_type, is_training=False)
    c_c1, _, _, rec_c1 = enc_dec(input_c, AE_type, is_training=False)
...
rec_loss = rec_loss_a + rec_loss_b + rec_loss_c
...
if AE_type == "VAE":
    ...
    kld_loss = kld_loss_a + kld_loss_b + kld_loss_c
else:
...
adv_loss = adv_loss_b + adv_loss_c
...
D_loss = wd_loss + wd_loss1 + (gp_loss + gp_loss1) * delta


# otpimizers
d_vars = utils1.trainable_variables('discriminator')
g_vars = utils1.trainable_variables(
    ['Encoder', 'Decoder_a', 'Decoder_b', 'Decoder_c'])
    ...
    for ep in range(n_epochs):
        for it in range(iters_per_epoch):
            ...
            D_summary_opt, _ = sess.run([D_summary, D_step],
                                        feed_dict={input_a: t_batch, input_b: s_batch, input_c: s1_batch})
