...
def get_data(path, data_type, use_test):
    ...
    source1_train_data_filename = path+"/source1_train_data.csv"
    source2_train_data_filename = path+"/source2_train_data.csv"
    source_test_data_filename = path+"/source_test_data.csv"
    target_test_data_filename = path+"/target_test_data.csv"
    source1_test_data_filename = path+"/source1_test_data.csv"
    source2_test_data_filename = path+"/source2_test_data.csv"

    source_train_data = np.loadtxt(source_train_data_filename, delimiter=',')
    source1_train_data = np.loadtxt(source1_train_data_filename, delimiter=',')
    source2_train_data = np.loadtxt(source2_train_data_filename, delimiter=',')
    target_train_data = np.loadtxt(
        target_train_data_filename, delimiter=',')
    min_n = np.min([len(source_train_data), len(
        target_train_data), len(source1_train_data), len(source2_train_data)])
    source_train_data[isnan(source_train_data)] = 0
    source1_train_data[isnan(source1_train_data)] = 0
    source2_train_data[isnan(source2_train_data)] = 0
    ...
    if use_test & os.path.isfile(source1_test_data_filename):
        print('using source1 test data: '+source1_test_data_filename)
        source1_test_data = np.loadtxt(
            source1_test_data_filename, delimiter=',')
        source1_test_data[isnan(source1_test_data)] = 0

    else:
        source1_test_data = source1_train_data
        print('using same source1  data for training and testing')

    if use_test & os.path.isfile(source2_test_data_filename):
        print('using source2 test data: '+source2_test_data_filename)
        source2_test_data = np.loadtxt(
            source2_test_data_filename, delimiter=',')
        source2_test_data[isnan(source2_test_data)] = 0

    else:
        source2_test_data = source2_train_data
        print('using same source2  data for training and testing')
    ...
    if data_type == 'cytof':
        ...
        source1_train_data = preProcessCytofData(source1_train_data)
        source1_test_data = preProcessCytofData(source1_test_data)
        source2_train_data = preProcessCytofData(source2_train_data)
        source2_test_data = preProcessCytofData(source2_test_data)

        target_train_data = preProcessCytofData(target_train_data)
        target_test_data = preProcessCytofData(target_test_data)

    source_train_data, source_test_data, _ = standard_scale(
        source_train_data, source_test_data)
    source1_train_data, source1_test_data, _ = standard_scale(
        source1_train_data, source1_test_data)
    source2_train_data, source2_test_data, _ = standard_scale(
        source2_train_data, source2_test_data)

    target_train_data, target_test_data, preprocessor = standard_scale(
        target_train_data, target_test_data)

    return source_train_data, target_train_data, source1_train_data,  source2_train_data, source_test_data, target_test_data, source1_test_data, source2_test_data, min_n, preprocessor
...
def make_dataset(data, batch_size=100, buffer_size=4096):
    ...
    dataset = dataset.apply(tf.contrib.data.batch_and_drop_remainder(
        batch_size))  # instead of previous row
...
def gradient_penalty(real, fake, f):
    def interpolate(a, b):
        shape = tf.concat((tf.shape(a)[0:1], tf.tile(
            [1], [a.shape.ndims - 1])), axis=0)
...
def tensors_filter(tensors, filters, combine_type='or'):
    assert isinstance(tensors, (list, tuple)
                      ), '`tensors` shoule be a list or tuple!'
    ...
    def __init__(self,
        ...
        if scales == None:
            ...
            for ii in range(1, 20):
                sample = MMDTargetTrain[np.random.randint(
                    MMDTargetTrain.shape[0], size=MMDTargetSampleSize), :]
